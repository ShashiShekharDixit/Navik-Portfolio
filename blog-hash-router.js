/* ═══════════════════════════════════════════════════════════
   navik — Blog Hash Router
   Manages blog posts from backend with hash-based routing
   Works with file:// protocol using #/blog routing
   ═══════════════════════════════════════════════════════════ */

class BlogHashRouter {
  constructor() {
    this.currentPage = 1;
    this.itemsPerPage = 6;
    this.allPosts = [];
  ...
  init() {
    ...
    // Only initialize blog features if on blog page
    if (this.isBlogPage) {
      ...
      // If navigating to #/blog from another page, redirect to blog.html
  ...
  checkBlogPage() {
    ...
    // Check if we're on blog.html or navigating via #/blog
    // Also check for blog-hero-section element
  ...
  handleBlogNavigation() {
    const hash = window.location.hash;
    if (hash === '#/blog' || hash === '#/blog/') {
      // If not on blog.html, redirect to it
      if (!window.location.pathname.includes('blog.html')) {
        window.location.href = 'blog.html';
      } else {
        // We're already on blog.html, reload content
        this.setupEventListeners();
        this.fetchBlogPosts();
      }
    }
  ...
  setupEventListeners() {
    // Category buttons (Blog Tabs)
    const categoryButtons = document.querySelectorAll('.blog-tab');
    categoryButtons.forEach(btn => {
      btn.addEventListener('click', (e) => {
        const target = e.target.closest('.blog-tab');
        if (target) {
  ...
  async fetchBlogPosts() {
    try {
      ...
      // Note: Date should be stored as either a Date object or ISO string in the backend
      const mockData = [
        {
          id: 1,
          ...
          author: 'Navik Team',
          date: new Date(2026, 5, 29), // June 29, 2026
          readTime: 4,
  ...
  renderBlogPosts() {
    ...
    if (postsToShow.length === 0) {
      container.innerHTML = '<p class="no-posts" style="grid-column: 1/-1; text-align: center; padding: 60px 20px;">No articles found. Try a different search or category.</p>';
    ...
    setTimeout(() => {
      container.innerHTML = postsToShow.map(post => {
        ...
        return `
        <article class="blog-post-card" itemscope itemtype="https://schema.org/BlogPosting" data-post-id="${post.id}" data-content="${escapedContent}">
          <a href="#/blog/${post.slug}" class="blog-post-image-link">
            <img src="${post.image}" alt="${post.title}" class="blog-post-image" itemprop="image">
            <span class="blog-post-category">${this.getCategoryLabel(post.category)}</span>
          </a>
          
          <div class="blog-post-content">
            <h2 itemprop="headline">
              <a href="#/blog/${post.slug}" class="blog-post-title">${post.title}</a>
            </h2>
            
            <p class="blog-post-excerpt" itemprop="description">${post.excerpt}</p>
            
            <div class="blog-post-meta">
              <span class="blog-post-author" itemprop="author">${post.author}</span>
              <span class="blog-post-date" itemprop="datePublished" title="${this.formatDate(post.date)}">${this.getRelativeDate(post.date)}</span>
              <span class="blog-post-readtime">${post.readTime} min</span>
            </div>
          ...
          <!-- Hidden SEO metadata -->
          <meta itemprop="articleBody" content="${escapedContent}">
  ...
  updateResultsInfo() {
    const resultsText = document.getElementById('blogResultsText');
    if (!resultsText) return;

    const total = this.filteredPosts.length;
    const categoryName = this.getCategoryLabel(this.currentCategory);
    
    let text = '';
    if (this.searchQuery) {
      text = `Found ${total} article${total !== 1 ? 's' : ''} matching "${this.searchQuery}"`;
    } else if (this.currentCategory !== 'all') {
      text = `${categoryName} (${total} article${total !== 1 ? 's' : ''})`;
    } else {
      text = `Showing ${total} article${total !== 1 ? 's' : ''}`;
    }
  ...
  showErrorMessage() {
    const container = document.getElementById('blogPostsContainer');
    if (container) {
      container.innerHTML = '<p class="error-message" style="grid-column: 1/-1; text-align: center; padding: 60px 20px; color: #ef4444;">Unable to load blog posts. Please try again later.</p>';
    }
  }
}

// Initialize blog router when DOM is ready
